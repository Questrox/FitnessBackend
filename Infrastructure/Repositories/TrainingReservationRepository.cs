...
namespace Infrastructure.Repositories
{
    public class TrainingReservationRepository : Repository<TrainingReservation>, ITrainingReservationRepository
    {
        ...
        public async Task<IEnumerable<TrainingReservation>> GetReservationsByTrainingIdAsync(int trainingId)
            => await _dbSet.Include(t => t.Client).ThenInclude(c => c.User)
            .Include(t => t.ReservationStatus)
            .Where(t => t.ReservationStatusId != (int)ReservationStatusEnum.Cancelled && t.TrainingId == trainingId)
            .ToListAsync();
    }
}
